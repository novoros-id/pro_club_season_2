...
def image_is_required(paragraph):
    ...
    "Иллюстрация нужна, если в тексте:\n"
    "- описывается конкретный элемент интерфейса (кнопка, меню, поле ввода и т.д.),\n"
    "- даётся пошаговое руководство с действиями пользователя (нажать, выбрать, перейти, открыть и т.п.),\n"
    "- упоминается визуальный результат («вы увидите», «на экране появится», «как показано на рисунке»),\n"
    "- есть ссылка на расположение чего-либо («в левом верхнем углу», «под заголовком» и т.п.).\n\n"
    "Если иллюстрация **полезна или необходима** для понимания — ответь «1».\n"
    "Если текст носит общий, описательный или теоретический характер — ответь «0».\n\n"
